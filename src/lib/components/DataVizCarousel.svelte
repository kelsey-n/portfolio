<script>
	...
	let images = [
		...
		},
		{
			src: '/images/PLACEHOLDER.png',
			title: 'Image 1',
			content: { title: 'Content for image 1', images: ['/images/PLACEHOLDER.png'] }
		},
		{
			src: '/images/PLACEHOLDER.png',
			title: 'Image 2',
			content: { title: 'Content for image 2', images: ['/images/PLACEHOLDER.png'] }
		},
		{
			src: '/images/PLACEHOLDER.png',
			title: 'Image 3',
			content: { title: 'Content for image 3', images: ['/images/PLACEHOLDER.png'] }
		},
		{
			src: '/images/PLACEHOLDER.png',
			title: 'Image 4',
			content: { title: 'Content for image 4', images: ['/images/PLACEHOLDER.png'] }
		},
		{
			src: '/images/PLACEHOLDER.png',
			title: 'Image 4',
	...
	// tweak: for dims calc, use max of span (for overlapping words) and work label div - may not work for opp direction scrolling though
	...
	// Try using borders to get borders for year sublabels
	...
	onMount(() => {
		...
		const workLabelSpacingElements = document.querySelectorAll(
			'.minimap-work-labels .minimap-work-label-space'
		);
		const workLabelElements = document.querySelectorAll('.minimap-work-label');
		const workLabelTextSpans = document.querySelectorAll('.minimap-work-label span'); // using spans lets us calculate the dims of text that overflows the div
		// let activeWorkLabelElementIndex = 0;
		...
		const initialDims = [...workLabelTextSpans].map((d) => d.getBoundingClientRect());

		function stickyTranslate(initialDims, workLabelElements, workLabelTextSpans, type) {
			// activeLabelIndices[type]

			// let activeLabelIndices[type] = type === 'work' ? activeWorkLabelElementIndex : null;
			// let activeLabelIndices[type] = activeLabelIndices[type];

			// Updating dimensions of all labels
			const currentDims = [...workLabelTextSpans].map((d) => d.getBoundingClientRect());
			...
			if (
				...
				workLabelElements.forEach((workLabel, i) => {
					if (
						...
						// );
						let currentTranslateCapped = Math.max(
							Math.min(currentTranslate + getDistanceBetweenElems(initialDims, i, i + 1), 0),
			...
			if (activeLabelIndices[type] < currentDims.length - 1) {
				if (currentDims[activeLabelIndices[type] + 1].left <= initialDims[0].left) {
		...
		function animate() {
			...
			if (Math.abs(currentTranslate - targetTranslate) > 0.01) {
				...
				stickyTranslate(initialDims, workLabelElements, workLabelTextSpans, 'work');
...
<div class="minimap-container">
	...
	<div class="minimap">
		...
		<div class="minimap-work-labels">
			{#each images as image, i}
				<div class="minimap-work-label-space" id="minimap-work-label-{i}">
					{#if i === 0}<div class="minimap-work-label">
							<span>HIGHLIGHTED long label WORK</span>
						</div>{/if}
					{#if i === 3}<div class="minimap-work-label"><span>ALL WORK</span></div>{/if}
					{#if i === 6}<div class="minimap-work-label"><span>TEST</span></div>{/if}
				</div>
			{/each}
		</div>
		<div class="items">
